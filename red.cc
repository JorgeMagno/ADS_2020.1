...
main (int argc, char *argv[])
{
  ...
  // Install PPBP on all right side nodes
  uint16_t port = 5001;
  ...
  srand(time(NULL));
  ApplicationContainer novoId;

  for (uint32_t i = 0; i < d.LeftCount (); ++i)
    {
      // Create sending packets to the Right side
      AddressValue remoteAddress (InetSocketAddress (d.GetRightIpv4Address (i), port));
      clientHelper.SetAttribute ("Remote", remoteAddress);
      novoId.Add (clientHelper.Install (d.GetLeft (i)));
    }
  double client = randf(MIN, MAX);
  novoId.Start (Seconds (client));
  novoId.Stop (Seconds (client_stop_time));
